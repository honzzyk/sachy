...
public class PGNWriter extends AbstractPGNWriter {
	...
	/**
	 * Creates a new {@code XMLPGNWriter} with a specific name and a Level for
	 * the Logger.
	 * 
	 * @param loggerLevel
	 *            the {@code Level} of the Logger. It could be:<br>
	...
	/**
	 * Creates a new {@code XMLPGNWriter} with a specific name, a Level for the
	 * Logger and an xml configuration file.
	 * 
	 * @param loggerLevel
	 *            the {@code Level} of the Logger. It could be:<br>
	 *            <ul>
	 ...
	 *            the xml configuration file used to configure the Logger.
	 */
	 ...
	 * 
	 * @see
	 * com.supareno.pgnparser.Writer#writePGNGame(com.supareno.pgnparser.PGNGame
	 * )
	 */
	public boolean writePGNGame(Game game) {
		if (game == null) {
			throw new NullPointerException(
					"the PGNGame or the file name is null");
		}
	...
	public String writePGNGameToString(Game game) {
		if (game == null) {
			throw new NullPointerException(
					"the PGNGame  is null");
		}
		Games games = new Games();
		games.addGame(game);
		return writePGNGamesToString(games);
	}
	
	public String writePGNGamesToString(Games games) {
		...
		for (Game game : games.getGame()) {
			...
			s += "\n";
			// writing attributes
			s +=getAttributesStringRepresentation(game);
			// writing hits & result
			List<Hit> hits = game.getHits().getHit();
			if (hits != null && hits.size() > 0) {
				for (Hit hit : hits) {
					s += hit.getNumber() + "." + hit.getContent();
					s += " ";
				}
			}
		}
